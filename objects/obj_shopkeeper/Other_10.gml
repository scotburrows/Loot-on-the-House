...
if (!instance_exists(obj_dialogue)) {
	if (obj_player.has_painting or obj_player.has_silverware or obj_player.has_perfume or obj_player.has_console or obj_player.has_ring or obj_player.has_wine or obj_player.has_vault or obj_player.has_speakers or obj_player.has_pinkbear or obj_player.has_bluebear) {
		var money = 0;
		if (obj_player.has_painting) money += 20;
		if (obj_player.has_silverware) money += 10;
		if (obj_player.has_perfume) money += 15;
		if (obj_player.has_console) money += 10;
		if (obj_player.has_ring) money += 20;
		if (obj_player.has_wine) money += 20;
		if (obj_player.has_vault) money += 10;
		if (obj_player.has_speakers) money += 15;
		if (obj_player.has_pinkbear) money += 5;
		if (obj_player.has_bluebear) money += 5;
		global.money += money;
